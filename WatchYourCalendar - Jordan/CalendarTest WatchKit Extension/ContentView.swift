    ...
    func timeTravelComponent() -> some View {
        return Group {
            ...
            Button(action: { opacity = 0; delayWithSeconds(0.3) { opacity = 1 }; delayWithSeconds(0.2) { minOffset = 0 } }) {
                Text("Return to Present").fontWeight(.heavy).font(.system(size: 16))
            }
        }
    }
    ...
    var body: some View {
        VStack{
            if globalOffset == 0 && minOffset != 0 && cycleDay != 0{
                ...
                Spacer()
                if globalOffset == 0 {cycleDayDay().font(.title).fontWeight(.heavy).multilineTextAlignment(.center)} else {
                    cycleDayDay().font(.system(size: 30)).fontWeight(.heavy).multilineTextAlignment(.center)
                }
